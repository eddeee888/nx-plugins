...
describe('nx-dev-tools generator', () => {
  ...
  it('should generate files correctly', async () => {
    ...
    const core = tree.read('dev-tools/bin/core.sh', 'utf-8');
    ...
    const buildDevImages = tree.read('dev-tools/bin/dc-build-dev-images.sh', 'utf-8');
    expect(buildDevImages).toMatchInlineSnapshot(`
      "#!/bin/bash
      ...
      function build_dev_images {
        ...
        docker-compose \\\\
          --file dev-tools/docker-image/build-dev-image.yml \\\\
