...
const setupNetwork = (projectConfig: ProjectConfiguration): void => {
  const { primaryDomain } = readDevToolsConfig(projectConfig.root);
  const hostResolverFile = getHostResolverFile(primaryDomain);
...
const getHostResolverFile = (primaryDomain: string): string => {
  const resolverDir = '/etc/resolver';
  return path.join(resolverDir, primaryDomain);
};
